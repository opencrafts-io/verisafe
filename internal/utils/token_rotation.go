	...
	for {
		...
		case <-ticker.C:
			if err := trs.checkTokensNeedingRotation(ctx); err != nil {
				trs.logger.Error(
					"Failed to check tokens needing rotation",
					slog.String("error", err.Error()),
				)
			}
		}
...
func (trs *TokenRotationScheduler) cleanupExpiredTokens(
	ctx context.Context,
) error {
...
func (trs *TokenRotationScheduler) checkTokensNeedingRotation(
	ctx context.Context,
) error {
...
func (trs *TokenRotationScheduler) GetTokensNeedingRotation(
	ctx context.Context,
...
func (trs *TokenRotationScheduler) RotateToken(
	ctx context.Context,
	tokenID string,
	newTokenHash string,
	newExpiry *time.Time,
