...
func (h *harness) request(caps ...providers.Capability) grants.AccessTokenRequest {
	return grants.AccessTokenRequest{
		AccountID:    h.accountID,
		Provider:     "google",
		Capabilities: caps,
	}
...
func TestGetAccessToken_UnknownProvider(t *testing.T) {
	...
	_, err := h.svc.GetAccessToken(context.Background(), grants.AccessTokenRequest{
		AccountID: h.accountID,
		Provider:  "microsoft",
	})
...
func TestGetAccessToken_NoGrant(t *testing.T) {
	...
	assert.ErrorIs(t, err, grants.ErrNoGrant)
	assert.ErrorIs(t, err, core.ErrNotFound)
}
...
func TestGetAccessToken_RevokedGrant(t *testing.T) {
	...
	_, err := h.svc.GetAccessToken(context.Background(), h.request(providers.CapabilityCalendar))
	assert.ErrorIs(t, err, grants.ErrGrantRevoked)
	assert.Equal(t, 0, h.exchanger.refreshCalls)
}
...
func TestGetAccessToken_ProviderOutageNeverRevokes(t *testing.T) {
	...
	_, err := h.svc.GetAccessToken(context.Background(), h.request(providers.CapabilityCalendar))
	assert.ErrorIs(t, err, grants.ErrProviderUnavailable)
...
func TestGetAccessToken_RefreshUnsupportedProvider(t *testing.T) {
	...
	assert.ErrorIs(t, err, grants.ErrRefreshUnsupported)
	assert.Equal(t, 0, h.exchanger.refreshCalls)
}
...
func TestGetAccessToken_ServedFromCache(t *testing.T) {
	...
	got, err := svc.GetAccessToken(context.Background(), grants.AccessTokenRequest{
		AccountID:    accountID,
		Provider:     "google",
...
func TestRecordGrant_PreservesExistingRefreshTokenWhenProviderOmitsIt(t *testing.T) {
	...
	// disconnect the user.
	err := h.svc.RecordGrant(context.Background(), grants.RecordGrantInput{
		AccountID:      h.accountID,
		Provider:       "google",
...
func TestErrInsufficientScope_MapsToForbidden(t *testing.T) {
	err := &grants.ErrInsufficientScope{Provider: "google", MissingScopes: []string{googleCalendar}}
	assert.ErrorIs(t, err, core.ErrForbidden)
...
func TestSentinels_MapToDistinctStatuses(t *testing.T) {
	...
	assert.ErrorIs(t, grants.ErrNoGrant, core.ErrNotFound)
	assert.ErrorIs(t, grants.ErrGrantRevoked, core.ErrForbidden)
	assert.ErrorIs(t, grants.ErrRefreshUnsupported, core.ErrConflict)
	assert.ErrorIs(t, grants.ErrProviderUnavailable, core.ErrUnavailable)
}
