...
func (h *OAuthBrokerHandler) RegisterHandlers(router *http.ServeMux) {
	router.Handle("POST /oauth/{provider}/token",
		...
		)(AppHandler(h.IssueProviderToken)),
	)
	...
	router.Handle("GET /oauth/grants",
		middleware.CreateStack(
			middleware.IsAuthenticated(h.Cfg, h.DB, h.Cacher, h.Logger),
			middleware.HasPermission([]string{"read:provider_token:any"}),
		)(AppHandler(h.ListAccountGrants)),
	)
...
// @Param        request   body      handlers.ProviderTokenRequest  true  "Account and capabilities required"
// @Success      200  {object}  handlers.ProviderTokenResponse
...
// @Failure      403  {object}  handlers.InsufficientScopeResponse  "Caller is not a service token, lacks the permission, or the user has not granted the capability"
...
func (h *OAuthBrokerHandler) IssueProviderToken(
	...
	var result *service.AccessTokenResult
	err = h.withGrantService(r, func(svc service.GrantService) error {
		var innerErr error
		result, innerErr = svc.GetAccessToken(r.Context(), service.AccessTokenRequest{
			AccountID:    accountID,
			Provider:     provider,
...
// @Success      200  {array}   service.GrantView
...
func (h *OAuthBrokerHandler) ListAccountGrants(
	...
	var grants []service.GrantView
	err = h.withGrantService(r, func(svc service.GrantService) error {
		var innerErr error
		grants, innerErr = svc.ListGrants(r.Context(), accountID)
		return innerErr
	})
	if err != nil {
		return err
	...
	core.WriteJSON(w, http.StatusOK, grants)
	return nil
}
...
// @Produce      json
// @Param        provider  path  string  true  "OAuth2 provider"  Enums(google, spotify)
// @Param        request   body  handlers.ProviderTokenRequest  true  "Account to reconcile (capabilities ignored)"
...
func (h *OAuthBrokerHandler) writeTokenError(
	...
	if errors.Is(err, service.ErrGrantRevoked) {
	...
	if errors.Is(err, service.ErrNoGrant) {
...
func (h *OAuthBrokerHandler) withGrantService(
	r *http.Request,
	fn func(svc service.GrantService) error,
) error {
	...
	return core.WithTransaction(r.Context(), conn, func(tx pgx.Tx) error {
		return fn(service.NewGrantService(
			repository.New(tx),
			h.Cacher,
			h.Registry,
			h.Sealer,
