...
func TestStreakHandler_ConnectionAcquisitionFailure(t *testing.T) {
	h := &streak.StreakHandler{
		Logger: slog.New(slog.NewTextHandler(io.Discard, nil)),
		DB:     testsupport.FailingAcquireDB(t, errors.New("pool exhausted")),
	}

	req := httptest.NewRequest("GET", "/streaks/milestone/active", nil)

	testsupport.WireCase{
		Handler:         http.HandlerFunc(h.GetAllActiveStreakAchievements),
		Request:         req,
		WantStatus:      500,
