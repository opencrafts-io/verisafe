...
type ScopesResponse struct {
	Grants []service.GrantView `json:"grants"`
...
func (h *OAuthScopeHandler) RegisterHandlers(router *http.ServeMux) {
	router.Handle("POST /oauth/{provider}/authorize",
		...
		)(AppHandler(h.StartScopeUpgrade)),
	)
	...
	router.Handle("GET /oauth/{provider}/callback",
		AppHandler(h.CompleteScopeUpgrade),
	)
	...
	router.Handle("DELETE /oauth/{provider}/grant",
		middleware.CreateStack(
			middleware.IsAuthenticated(h.Cfg, h.DB, h.Cacher, h.Logger),
			middleware.HasPermission([]string{"manage:oauth_grant:own"}),
		)(AppHandler(h.DisconnectProvider)),
	)
}
...
// @Param        request   body      handlers.ScopeAuthorizeRequest  true  "Capabilities to request and where to return the user"
// @Success      200  {object}  handlers.ScopeAuthorizeResponse
...
func (h *OAuthScopeHandler) CompleteScopeUpgrade(
	...
	err = h.withGrantService(r, func(svc service.GrantService) error {
		return svc.RecordGrant(r.Context(), service.RecordGrantInput{
...
// @Tags         oauth
// @Produce      json
// @Success      200  {object}  handlers.ScopesResponse
// @Failure      401  {object}  core.APIError  "Missing or invalid claims"
// @Security     BearerToken
...
func (h *OAuthScopeHandler) currentGrant(
	...
	accountID uuid.UUID,
	provider string,
) (*service.GrantView, error) {
	var grant *service.GrantView
