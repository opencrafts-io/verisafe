...
func brokerHandler() *oauth.OAuthBrokerHandler {
	cfg := &config.Config{}
	cfg.AuthenticationConfig.AuthAddress = "https://verisafe.example.com"

	return &oauth.OAuthBrokerHandler{
		Cfg:      cfg,
...
func TestInsufficientScopeResponse_GoldenShape(t *testing.T) {
	body := oauth.InsufficientScopeResponse{
...
func TestReauthorizationRequiredResponse_GoldenShape(t *testing.T) {
	raw, err := json.Marshal(oauth.ReauthorizationRequiredResponse{
...
func TestProviderTokenResponse_HasNoRefreshToken(t *testing.T) {
	raw, err := json.Marshal(oauth.ProviderTokenResponse{
		Provider:    "google",
